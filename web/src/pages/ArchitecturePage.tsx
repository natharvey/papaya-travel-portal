...
const NODE_DETAILS: Record<string, NodeDetail> = {
  ...
  unsplash: {
    whatIsIt: "Unsplash provides the activity photos displayed in the itinerary timeline. The API queries Unsplash using search terms generated by the Generator agent at itinerary creation time — the query is crafted alongside the activity itself, not derived from it after the fact.",
    whatItDoes: [
      "Supplies a high-quality photo for each morning, afternoon, and evening activity block in the itinerary",
      "Photo search queries are written by the Generator agent to be visually specific and distinct within each day (e.g. 'Fushimi Inari torii gates', 'Nishiki Market street food', 'Kyoto kaiseki dinner') — avoiding generic or repeated imagery",
      "Every returned photo passes through a Claude Haiku vision quality gate before display — if the image does not depict the activity, it is rejected and a fallback is used instead",
    ],
    whyThisChoice: "Unsplash has broad coverage of travel destinations and requires no per-image attribution on the API plan. The vision quality gate addresses the primary failure mode — visually irrelevant photos passing through keyword search.",
  },
  aerodatabox: {
...
const initialNodes = [
  ...
  { id: 'mapbox',      type: 'service', position: { x: 1190, y: 210 }, data: { label: 'Mapbox', sublabel: 'Interactive maps', icon: 'Map', color: EXT_COLOR } },
  { id: 'places',      type: 'service', position: { x: 1190, y: 360 }, data: { label: 'Google Places', sublabel: 'Hotel photos + geocoding', icon: 'Search', color: EXT_COLOR } },
  { id: 'unsplash',    type: 'service', position: { x: 1190, y: 510 }, data: { label: 'Unsplash', sublabel: 'Activity photos', icon: 'Image', color: EXT_COLOR } },
  { id: 'aerodatabox', type: 'service', position: { x: 1190, y: 660 }, data: { label: 'AeroDataBox', sublabel: 'Flight lookup', icon: 'Plane', color: EXT_COLOR } },
  { id: 'gmail',       type: 'service', position: { x: 1190, y: 810 }, data: { label: 'Gmail SMTP', sublabel: 'Transactional email', icon: 'Mail', color: EXT_COLOR } },
  { id: 'sentry',      type: 'service', position: { x: 1190, y: 960 }, data: { label: 'Sentry', sublabel: 'Error tracking', icon: 'AlertCircle', color: EXT_COLOR } },
]
